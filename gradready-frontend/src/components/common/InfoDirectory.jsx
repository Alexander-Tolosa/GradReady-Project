  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-5">
        <div>
          <h2 className="text-lg font-semibold text-white">Office Directory</h2>
          <p className="text-sm text-zinc-500 mt-0.5">Find office locations, contact details, and operating hours</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {filteredOffices.map((office, index) => (
          <div key={office.id} className="card p-4 animate-fade-in" style={{ animationDelay: `${index * 40}ms` }} id={`office-${office.id}`}>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-zinc-400 hover:text-zinc-300 transition-colors cursor-pointer">{office.email}</span>
              </div>
            </div>
          </div>
        ))}
